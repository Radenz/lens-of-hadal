            ...
            if (serializationSettingsArea.BeginFade())
            {
                ...
                if (GUILayout.Button("Load from file"))
                {
                    ...
                    if (path != "")
                    {
                        try
                        {
                            byte[] bytes = Serialization.AstarSerializer.LoadFromFile(path);
        ...
        void DeserializeGraphs(byte[] bytes)
        {
            try
            {
                ...
                for (int i = 0; i < graphEditors.Length; i++)
                {
                    ...
                    if (data != null) Serialization.TinyJsonDeserializer.Deserialize(data, graphEditors[i].GetType(), graphEditors[i], script.gameObject);
                }
            }
            catch (System.Exception e)
            {
